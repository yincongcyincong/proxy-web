...
func NewUDP() services2.Service {
	return &UDP{
		outPool: utils2.OutConn{},
		p:       utils2.NewConcurrentMap(),
		isStop:  false,
	}
}
...
func (s *UDP) OutToTCP(packet []byte, localAddr, srcAddr *net.UDPAddr) (err error) {
	...
	if isNew {
		go func() {
			...
			for {
				...
				srcAddrFromConn, body, err := utils2.ReadUDPPacket(bufio.NewReader(conn))
	...
	writer.Write(utils2.UDPPacket(srcAddr.String(), packet))
	err = writer.Flush()
	if err != nil {
