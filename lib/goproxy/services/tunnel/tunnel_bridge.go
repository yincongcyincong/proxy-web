...
func NewTunnelBridge() services2.Service {
	return &TunnelBridge{
		cfg:                TunnelBridgeArgs{},
...
func (s *TunnelBridge) callback(inConn net.Conn) {
	...
	var connType uint8
	err = utils2.ReadPacket(reader, &connType)
	if err != nil {
	...
	case CONN_SERVER:
		...
		err = utils2.ReadPacketData(reader, &key, &ID, &clientLocalAddr, &serverID)
		if err != nil {
			s.log.Printf("read error,ERR:%s", err)
			return
		}
		packet := utils2.BuildPacketData(ID, clientLocalAddr, serverID)
	...
	case CONN_CLIENT:
		...
		err = utils2.ReadPacketData(reader, &key, &ID, &serverID)
		if err != nil {
			s.log.Printf("read error,ERR:%s", err)
			return
		}
		...
		utils2.IoBind(*serverConn, inConn, func(err interface{}) {
	...
	case CONN_CLIENT_CONTROL:
		var key string
		err = utils2.ReadPacketData(reader, &key)
		if err != nil {
			s.log.Printf("read error,ERR:%s", err)
			return
		}
