...
func NewTunnelServerManager() services2.Service {
...
func (s *TunnelServerManager) GetOutConn(typ uint8) (outConn net.Conn, ID string, err error) {
	...
	ID = s.serverID
	_, err = outConn.Write(utils2.BuildPacket(typ, s.serverID))
	if err != nil {
...
func NewTunnelServer() services2.Service {
	return &TunnelServer{
		cfg:       TunnelServerArgs{},
		udpChn:    make(chan UDPItem, 50000),
		isStop:    false,
...
func (s *TunnelServer) GetOutConn(typ uint8) (outConn net.Conn, ID string, err error) {
	...
	ID = utils2.Uniqueid()
	_, err = outConn.Write(utils2.BuildPacket(typ, *s.cfg.Key, ID, remoteAddr, s.cfg.Mgr.serverID))
	if err != nil {
		s.log.Printf("write connection data err: %s ,retrying...", err)
		utils2.CloseConn(&outConn)
		return
