...
func NewHTTP() services2.Service {
	return &HTTP{
		outPool:        utils2.OutConn{},
		cfg:            HTTPArgs{},
		checker:        utils2.Checker{},
...
func (s *HTTP) InitService() (err error) {
	s.InitBasicAuth()
	if *s.cfg.Parent != "" {
		s.checker = utils2.NewChecker(*s.cfg.HTTPTimeout, int64(*s.cfg.Interval), *s.cfg.Blocked, *s.cfg.Direct, s.log)
	}
	if *s.cfg.DNSAddress != "" {
...
func (s *HTTP) callback(inConn net.Conn) {
	...
	var err interface{}
	var req utils2.HTTPRequest
	req, err = utils2.NewHTTPRequest(&inConn, 4096, s.IsBasicAuth(), &s.basicAuth, s.log)
	if err != nil {
		if err != io.EOF {
...
func (s *HTTP) OutToTCP(useProxy bool, address string, inConn *net.Conn, req *utils2.HTTPRequest) (err interface{}) {
	...
	for {
		...
		if useProxy {
			...
			outConn, err = utils2.ConnectHost(s.Resolve(address), *s.cfg.Timeout)
		}
		tryCount++
	...
	if req.IsHTTPS() && (!useProxy || *s.cfg.ParentType == "ssh") {
		...
		if (!useProxy || *s.cfg.ParentType == "ssh") && !req.IsSNI {
			_, err = outConn.Write(utils2.RemoveProxyHeaders(req.HeadBuf))
		} else {
