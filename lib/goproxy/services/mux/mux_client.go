...
func NewMuxClient() services2.Service {
	return &MuxClient{
		cfg:      MuxClientArgs{},
		isStop:   false,
	...
	for i := 1; i <= count; i++ {
		...
		go func(i int) {
			...
			for {
				...
				_, err = conn.Write(utils2.BuildPacket(CONN_CLIENT, fmt.Sprintf("%s-%d", *s.cfg.Key, i)))
				...
				for {
					...
					go func() {
						...
						err = utils2.ReadPacketData(stream, &ID, &clientLocalAddr, &serverID)
...
func (s *MuxClient) ServeUDP(inConn *smux.Stream, localAddr, ID string) {
	...
	for {
		...
		srcAddr, body, err := utils2.ReadUDPPacket(inConn)
...
func (s *MuxClient) processUDPPacket(inConn *smux.Stream, srcAddr, localAddr string, body []byte) {
	...
	bs := utils2.UDPPacket(srcAddr, respBody)
	(*inConn).SetDeadline(time.Now().Add(time.Millisecond * time.Duration(*s.cfg.Timeout)))
...
func (s *MuxClient) ServeConn(inConn *smux.Stream, localAddr, ID string) {
	...
	for {
		...
		i++
		outConn, err = utils2.ConnectHost(localAddr, *s.cfg.Timeout)
		if err == nil || i == 3 {
			break
		} else {
			if i == 3 {
