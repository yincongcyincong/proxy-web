...
type MuxBridge struct {
	cfg                MuxBridgeArgs
	clientControlConns utils2.ConcurrentMap
	serverConns        utils2.ConcurrentMap
	router             utils2.ClientKeyRouter
	l                  *sync.Mutex
	isStop             bool
...
func NewMuxBridge() services2.Service {
	b := &MuxBridge{
		cfg:                MuxBridgeArgs{},
		clientControlConns: utils2.NewConcurrentMap(),
		serverConns:        utils2.NewConcurrentMap(),
		l:                  &sync.Mutex{},
		isStop:             false,
	}
	b.router = utils2.NewClientKeyRouter(&b.clientControlConns, 50000)
	return b
}
...
func (s *MuxBridge) StopService() {
	...
	for _, g := range s.clientControlConns.Items() {
		for _, session := range g.(*utils2.ConcurrentMap).Items() {
...
func (s *MuxBridge) handler(inConn net.Conn) {
	...
	err = utils2.ReadPacket(reader, &connType, &key)
	...
	case CONN_SERVER:
		...
		inConn.SetDeadline(time.Now().Add(time.Millisecond * time.Duration(*s.cfg.Timeout)))
		err = utils2.ReadPacketData(reader, &serverID)
		inConn.SetDeadline(time.Time{})
		if err != nil {
			s.log.Printf("read error,ERR:%s", err)
			return
