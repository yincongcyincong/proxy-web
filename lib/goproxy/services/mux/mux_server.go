...
type MuxServerManager struct {
	...
	serverID string
	servers  []*services2.Service
	log      *logger.Logger
}

func NewMuxServerManager() services2.Service {
	return &MuxServerManager{
		cfg:      MuxServerArgs{},
		udpChn:   make(chan MuxUDPItem, 50000),
		serverID: utils2.Uniqueid(),
		servers:  []*services2.Service{},
	}
}
...
func NewMuxServer() services2.Service {
	return &MuxServer{
		cfg:      MuxServerArgs{},
		udpChn:   make(chan MuxUDPItem, 50000),
		lockChn:  make(chan bool, 1),
		sessions: utils2.NewConcurrentMap(),
		isStop:   false,
	}
}
...
func (s *MuxServer) Start(args interface{}, log *logger.Logger) (err error) {
	...
	s.sc = utils2.NewServerChannel(host, p, s.log)
	if *s.cfg.IsUDP {
...
func (s *MuxServer) GetOutConn() (outConn net.Conn, ID string, err error) {
	...
	_, err = outConn.Write(utils2.BuildPacketData(ID, remoteAddr, s.cfg.Mgr.serverID))
...
func (s *MuxServer) GetConn(index string) (conn net.Conn, err error) {
	...
	if !ok {
		...
		_, err = c.Write(utils2.BuildPacket(CONN_SERVER, *s.cfg.Key, s.cfg.Mgr.serverID))
...
func (s *MuxServer) getParentConn() (conn net.Conn, err error) {
	if *s.cfg.ParentType == "tls" {
		var _conn tls.Conn
		_conn, err = utils2.TlsConnectHost(*s.cfg.Parent, *s.cfg.Timeout, s.cfg.CertBytes, s.cfg.KeyBytes, nil)
		if err == nil {
			conn = net.Conn(&_conn)
		}
	} else if *s.cfg.ParentType == "kcp" {
		conn, err = utils2.ConnectKCPHost(*s.cfg.Parent, s.cfg.KCP)
	} else {
		conn, err = utils2.ConnectHost(*s.cfg.Parent, *s.cfg.Timeout)
	}
	return
}
func (s *MuxServer) UDPConnDeamon() {
	go func() {
		...
		RETRY:
					...
					if err != nil {
						...
						go func(outConn net.Conn, ID string) {
							...
							for {
								...
								outConn.SetDeadline(time.Now().Add(time.Millisecond * time.Duration(*s.cfg.Timeout)))
								srcAddrFromConn, body, err := utils2.ReadUDPPacket(outConn)
								outConn.SetDeadline(time.Time{})
								if err != nil {
			...
			_, err = outConn.Write(utils2.UDPPacket(item.srcAddr.String(), *item.packet))
