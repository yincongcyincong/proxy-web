...
type Socks struct {
	cfg                   SocksArgs
	checker               utils2.Checker
	basicAuth             utils2.BasicAuth
	sshClient             *ssh.Client
	lockChn               chan bool
	udpSC                 utils2.ServerChannel
	sc                    *utils2.ServerChannel
...
func NewSocks() services2.Service {
	return &Socks{
		cfg:                   SocksArgs{},
		checker:               utils2.Checker{},
		basicAuth:             utils2.BasicAuth{},
...
func (s *Socks) InitService() (err error) {
	...
	if *s.cfg.Parent != "" {
		s.checker = utils2.NewChecker(*s.cfg.Timeout, int64(*s.cfg.Interval), *s.cfg.Blocked, *s.cfg.Direct, s.log)
	}
	if *s.cfg.ParentType == "ssh" {
		e := s.ConnectSSH()
		if e != nil {
		...
		go func() {
			...
			for {
				...
				conn, err := utils2.ConnectHost(s.Resolve(*s.cfg.Parent), *s.cfg.Timeout*2)
				if err == nil {
...
func (s *Socks) Start(args interface{}, log *logger.Logger) (err error) {
	...
	sc := utils2.NewServerChannelHost(*s.cfg.Local, s.log)
...
func (s *Socks) socksConnCallback(inConn net.Conn) {
	...
	methodReq, err := socks2.NewMethodsRequest(inConn)
	inConn.SetReadDeadline(time.Time{})
	if err != nil {
		methodReq.Reply(socks2.Method_NONE_ACCEPTABLE)
	...
	if !s.IsBasicAuth() {
		if !methodReq.Select(socks2.Method_NO_AUTH) {
			methodReq.Reply(socks2.Method_NONE_ACCEPTABLE)
			utils2.CloseConn(&inConn)
		...
		//method select reply
		err = methodReq.Reply(socks2.Method_NO_AUTH)
		if err != nil {
		...
		//auth
		if !methodReq.Select(socks2.Method_USER_PASS) {
			methodReq.Reply(socks2.Method_NONE_ACCEPTABLE)
			utils2.CloseConn(&inConn)
		...
		err = methodReq.Reply(socks2.Method_USER_PASS)
		if err != nil {
			s.log.Printf("reply answer data fail,ERR: %s", err)
			utils2.CloseConn(&inConn)
			return
	...
	//request detail
	request, err := socks2.NewRequest(inConn)
	if err != nil {
	...
	switch request.CMD() {
	case socks2.CMD_BIND:
		//bind 不支持
		request.TCPReply(socks2.REP_UNKNOWN)
		utils2.CloseConn(&inConn)
		return
	case socks2.CMD_CONNECT:
		//tcp
		s.proxyTCP(&inConn, methodReq, request)
	case socks2.CMD_ASSOCIATE:
		//udp
...
func (s *Socks) proxyTCP(inConn *net.Conn, methodReq socks2.MethodsRequest, request socks2.Request) {
	...
	for {
		...
		if *s.cfg.Always {
			...
			if *s.cfg.Parent != "" {
				...
				useProxy := false
				if utils2.IsIternalIP(host, *s.cfg.Always) {
					useProxy = false
				} else {
					var isInMap bool
				...
				if useProxy {
					...
					outConn, err = utils2.ConnectHost(s.Resolve(request.Addr()), *s.cfg.Timeout)
				}
			} else {
				outConn, err = utils2.ConnectHost(s.Resolve(request.Addr()), *s.cfg.Timeout)
				useProxy = false
	...
	if err != nil {
		...
		request.TCPReply(socks2.REP_NETWOR_UNREACHABLE)
		return
	}

	s.log.Printf("use proxy %v : %s", useProxy, request.Addr())

	request.TCPReply(socks2.REP_SUCCESS)
...
func (s *Socks) getOutConn(methodBytes, reqBytes []byte, host string, handshake bool) (outConn net.Conn, err interface{}) {
	...
	case "tcp":
		if *s.cfg.ParentType == "tls" {
			...
			_outConn, err = utils2.TlsConnectHost(s.Resolve(*s.cfg.Parent), *s.cfg.Timeout, s.cfg.CertBytes, s.cfg.KeyBytes, nil)
			outConn = net.Conn(&_outConn)
		} else if *s.cfg.ParentType == "kcp" {
			outConn, err = utils2.ConnectKCPHost(s.Resolve(*s.cfg.Parent), s.cfg.KCP)
		} else {
			outConn, err = utils2.ConnectHost(s.Resolve(*s.cfg.Parent), *s.cfg.Timeout)
		}
		if err != nil {
...
func (s *Socks) IsDeadLoop(inLocalAddr string, host string) bool {
	...
	if inPort == outPort {
		...
		if *s.cfg.DNSAddress != "" {
			...
			outIPs, err = utils2.MyLookupIP(outDomain)
		}
		if err == nil {
		...
		interfaceIPs, err := utils2.GetAllInterfaceAddr()
