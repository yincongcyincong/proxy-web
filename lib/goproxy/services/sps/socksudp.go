...
func (s *SPS) proxyUDP(inConn *net.Conn, serverConn *socks2.ServerConn) {
	defer func() {
		if e := recover(); e != nil {
	...
	if *s.cfg.ParentAuth != "" {
		...
		client = socks2.NewClientConn(&outconn, "udp", serverConn.Target(), time.Millisecond*time.Duration(*s.cfg.Timeout), &socks2.Auth{User: a[0], Password: a[1]}, nil)
	} else {
		if !s.IsBasicAuth() && auth.Password != "" && auth.User != "" {
			client = socks2.NewClientConn(&outconn, "udp", serverConn.Target(), time.Millisecond*time.Duration(*s.cfg.Timeout), &auth, nil)
		} else {
			client = socks2.NewClientConn(&outconn, "udp", serverConn.Target(), time.Millisecond*time.Duration(*s.cfg.Timeout), nil, nil)
		}
	}
