...
import (
	...
	"errors"
	"fmt"
	services2 "github.com/yincongcyincong/proxy-web/lib/goproxy/services"
	kcpcfg2 "github.com/yincongcyincong/proxy-web/lib/goproxy/services/kcpcfg"
	utils2 "github.com/yincongcyincong/proxy-web/lib/goproxy/utils"
	conncrypt2 "github.com/yincongcyincong/proxy-web/lib/goproxy/utils/conncrypt"
	sni2 "github.com/yincongcyincong/proxy-web/lib/goproxy/utils/sni"
...
type SPS struct {
	outPool               utils2.OutConn
	cfg                   SPSArgs
	domainResolver        utils2.DomainResolver
	basicAuth             utils2.BasicAuth
	serverChannels        []*utils2.ServerChannel
	userConns             utils2.ConcurrentMap
	log                   *logger.Logger
	udpRelatedPacketConns utils2.ConcurrentMap
	udpLocalKey           []byte
	udpParentKey          []byte
}

func NewSPS() services2.Service {
	return &SPS{
		outPool:               utils2.OutConn{},
		cfg:                   SPSArgs{},
		basicAuth:             utils2.BasicAuth{},
		serverChannels:        []*utils2.ServerChannel{},
		userConns:             utils2.NewConcurrentMap(),
		udpRelatedPacketConns: utils2.NewConcurrentMap(),
	}
}
func (s *SPS) CheckArgs() (err error) {
	...
	if *s.cfg.ParentType == "tls" || *s.cfg.LocalType == "tls" {
		s.cfg.CertBytes, s.cfg.KeyBytes, err = utils2.TlsBytes(*s.cfg.CertFile, *s.cfg.KeyFile)
		if err != nil {
			return
		}
		if *s.cfg.CaCertFile != "" {
...
func (s *SPS) InitService() (err error) {
	if *s.cfg.DNSAddress != "" {
		(*s).domainResolver = utils2.NewDomainResolver(*s.cfg.DNSAddress, *s.cfg.DNSTTL, s.log)
	}
	s.InitOutConnPool()
	err = s.InitBasicAuth()
	return
...
func (s *SPS) Start(args interface{}, log *logger.Logger) (err error) {
	...
	for _, addr := range strings.Split(*s.cfg.Local, ",") {
		if addr != "" {
			...
			sc := utils2.NewServerChannel(host, p, s.log)
...
func (s *SPS) callback(inConn net.Conn) {
	...
	if *s.cfg.LocalCompress {
		inConn = utils2.NewCompConn(inConn)
	}
	if *s.cfg.LocalKey != "" {
		inConn = conncrypt2.New(inConn, &conncrypt2.Config{
...
func (s *SPS) OutToTCP(inConn *net.Conn) (err error) {
	...
	bInConn := utils2.NewBufferedConn(*inConn)
	//important
	...
	isSNI, _ := sni2.ServerNameFromBytes(h)
	*inConn = bInConn
	address := ""
	var auth socks2.Auth
	var forwardBytes []byte
	//fmt.Printf("%v", h)
	if utils2.IsSocks5(h) {
		if *s.cfg.DisableSocks5 {
			(*inConn).Close()
			return
		}
		//socks5 server
		var serverConn *socks2.ServerConn
		if s.IsBasicAuth() {
			serverConn = socks2.NewServerConn(inConn, time.Millisecond*time.Duration(*s.cfg.Timeout), &s.basicAuth, enableUDP, udpIP, nil)
		} else {
			serverConn = socks2.NewServerConn(inConn, time.Millisecond*time.Duration(*s.cfg.Timeout), nil, enableUDP, udpIP, nil)
		}
	...
	} else if utils2.IsHTTP(h) || isSNI != "" {
		if *s.cfg.DisableHTTP {
			(*inConn).Close()
			return
		}
		//http
		var request utils2.HTTPRequest
		(*inConn).SetDeadline(time.Now().Add(time.Millisecond * time.Duration(*s.cfg.Timeout)))
		if s.IsBasicAuth() {
			request, err = utils2.NewHTTPRequest(inConn, 1024, true, &s.basicAuth, s.log)
		} else {
			request, err = utils2.NewHTTPRequest(inConn, 1024, false, nil, s.log)
		}
		...
		if s.IsBasicAuth() {
			...
			if len(userpassA) == 2 {
				auth = socks2.Auth{User: userpassA[0], Password: userpassA[1]}
			}
		}
	}
	if err != nil || address == "" {
	...
	if *s.cfg.ParentCompress {
		outConn = utils2.NewCompConn(outConn)
	}
	if *s.cfg.ParentKey != "" {
		outConn = conncrypt2.New(outConn, &conncrypt2.Config{
			Password: *s.cfg.ParentKey,
		})
	}

	if *s.cfg.ParentAuth != "" || s.IsBasicAuth() {
		forwardBytes = utils2.RemoveProxyHeaders(forwardBytes)
	}
	...
	if *s.cfg.ParentServiceType == "http" {
		...
		if isHTTPS {
			...
			forwardBytes = utils2.InsertProxyHeaders(forwardBytes, string(pb.Bytes()))
			pb.Reset()
		...
		//socks client
		var clientConn *socks2.ClientConn
		if *s.cfg.ParentAuth != "" {
			a := strings.Split(*s.cfg.ParentAuth, ":")
			if len(a) != 2 {
				err = fmt.Errorf("parent auth data format error")
			...
			clientConn = socks2.NewClientConn(&outConn, "tcp", address, time.Millisecond*time.Duration(*s.cfg.Timeout), &socks2.Auth{User: a[0], Password: a[1]}, nil)
		} else {
			if !s.IsBasicAuth() && auth.Password != "" && auth.User != "" {
				clientConn = socks2.NewClientConn(&outConn, "tcp", address, time.Millisecond*time.Duration(*s.cfg.Timeout), &auth, nil)
			} else {
				clientConn = socks2.NewClientConn(&outConn, "tcp", address, time.Millisecond*time.Duration(*s.cfg.Timeout), nil, nil)
			}
		}
	...
	utils2.IoBind((*inConn), outConn, func(err interface{}) {
...
func (s *SPS) InitBasicAuth() (err error) {
	if *s.cfg.DNSAddress != "" {
		s.basicAuth = utils2.NewBasicAuth(&(*s).domainResolver, s.log)
	} else {
		s.basicAuth = utils2.NewBasicAuth(nil, s.log)
	}
	if *s.cfg.AuthURL != "" {
