...
import (
	...
	http2 "github.com/yincongcyincong/proxy-web/lib/goproxy/services/http"
	kcpcfg2 "github.com/yincongcyincong/proxy-web/lib/goproxy/services/kcpcfg"
	mux2 "github.com/yincongcyincong/proxy-web/lib/goproxy/services/mux"
	socks2 "github.com/yincongcyincong/proxy-web/lib/goproxy/services/socks"
	sps2 "github.com/yincongcyincong/proxy-web/lib/goproxy/services/sps"
	tcp2 "github.com/yincongcyincong/proxy-web/lib/goproxy/services/tcp"
	tunnel2 "github.com/yincongcyincong/proxy-web/lib/goproxy/services/tunnel"
	udp2 "github.com/yincongcyincong/proxy-web/lib/goproxy/services/udp"
	logger "log"
	"os"
	"path"
	"path/filepath"
...
func StartWithLog(serviceID, serviceArgsStr string, loggerCallback LogCallback) (errStr string) {
	//define  args
	tcpArgs := tcp2.TCPArgs{}
	httpArgs := http2.HTTPArgs{}
	tunnelServerArgs := tunnel2.TunnelServerArgs{}
	tunnelClientArgs := tunnel2.TunnelClientArgs{}
	tunnelBridgeArgs := tunnel2.TunnelBridgeArgs{}
	muxServerArgs := mux2.MuxServerArgs{}
	muxClientArgs := mux2.MuxClientArgs{}
	muxBridgeArgs := mux2.MuxBridgeArgs{}
	udpArgs := udp2.UDPArgs{}
	socksArgs := socks2.SocksArgs{}
	spsArgs := sps2.SPSArgs{}
	dnsArgs := DNSArgs{}
	kcpArgs := kcpcfg2.KCPConfigArgs{}
	//build srvice args
	...
	switch serviceName {
	case "http":
		services2.Regist(serviceID, http2.NewHTTP(), httpArgs, log)
	case "tcp":
		services2.Regist(serviceID, tcp2.NewTCP(), tcpArgs, log)
	case "udp":
		services2.Regist(serviceID, udp2.NewUDP(), udpArgs, log)
	case "tserver":
		services2.Regist(serviceID, tunnel2.NewTunnelServerManager(), tunnelServerArgs, log)
	case "tclient":
		services2.Regist(serviceID, tunnel2.NewTunnelClient(), tunnelClientArgs, log)
	case "tbridge":
		services2.Regist(serviceID, tunnel2.NewTunnelBridge(), tunnelBridgeArgs, log)
	case "server":
		services2.Regist(serviceID, mux2.NewMuxServerManager(), muxServerArgs, log)
	case "client":
		services2.Regist(serviceID, mux2.NewMuxClient(), muxClientArgs, log)
	case "bridge":
		services2.Regist(serviceID, mux2.NewMuxBridge(), muxBridgeArgs, log)
	case "socks":
		services2.Regist(serviceID, socks2.NewSocks(), socksArgs, log)
	case "sps":
		services2.Regist(serviceID, sps2.NewSPS(), spsArgs, log)
	case "dns":
		services2.Regist(serviceName, NewDNS(), dnsArgs, log)
	}
	_, err = services2.Run(serviceID, nil)
	if err != nil {
